...
class BuildSheet {
    ...
    calculateSaveBreakdowns() {
        ...
        if (chosenActions.includes("focus-defense")) {
            ...
            if (saveBonus > 0) {
                ...
                breakdowns.reflex.push(`Defense Focus (Alter): ${saveBonus}`);
                breakdowns.will.push(`Defense Focus (Alter): ${saveBonus}`);
            }
        }
    ...
    generateMasteryIcons(action, state, masteries) {
        ...
        // Hide mastery icons if the action's roll code doesn't contain "Mastery" or "masteryreplace"
        // Exception: Evolve action always shows mastery selection
        if (
            action.lookup !== "evolve" &&
            (!action.dice || (!action.roll.includes("Mastery") && !action.roll.includes("masteryreplace")))
        ) {
    ...
    updatePassiveModifiers(state) {
        ...
        sectionsToUpdate.forEach((sectionId) => {
            ...
            if (section) {
                ...
                passiveModSpans.forEach((span) => {
                    // Determine which modifier to apply based on the action type
                    const card = span.closest(".card");
                    if (card) {
                        // Get action ID from either card.id or card class
                        let actionId = card.id ? card.id.replace("final", "") : "";

                        // If no id, check classList for action ID
                        if (!actionId) {
                            const classList = Array.from(card.classList);
                            const actionClass = classList.find(c => c.endsWith("final"));
                            if (actionClass) {
                                actionId = actionClass.replace("final", "");
                            }
                        }
    ...
    autoSelectSingleMasteries(state, actions, masteries) {
        // For each action, check if it has only one applicable mastery
        // If so, automatically select it to populate MR and other fields

        const allActions = [...(state.chosenActions || []), "attack", "rush"];

        allActions.forEach((actionId) => {
            const action = actions.find((a) => a.lookup === actionId);
            if (!action || !action.masteries) return;

            // Skip if the action doesn't need mastery selection
            if (action.lookup === "rush") return;
            if (action.lookup !== "evolve" && (!action.dice || (!action.roll.includes("Mastery") && !action.roll.includes("masteryreplace")))) {
                return;
            }
            ...
            if (applicableMasteries.length === 1) {
                const masteryId = applicableMasteries[0];
                const masteryIndex = state.chosenMasteries.indexOf(masteryId);
                const mastery = masteries.find((m) => m.lookup === masteryId);
                ...
                if (masteryIndex !== -1 && mastery) {
                    ...
                    // Update the MR (mastery rank)
                    if (masteryReplace) {
                        masteryReplace.innerHTML = rankLetter;
                    }

                    // Update the mastery name
                    if (mnameReplace) {
...
function toggleUltra(actionId, suffix = "Ultra") {
    handleMutualExclusivity(actionId, "Ultra", ["Radial"]);
    toggleActionButton(actionId, suffix, "Ultra");
}

function toggleRadial(actionId, suffix = "Radial") {
    handleMutualExclusivity(actionId, "Radial", ["Ultra"]);
    toggleActionButton(actionId, suffix, "Radial");
}

function toggleAlly(actionId, suffix = "Ally") {
    handleMutualExclusivity(actionId, "Ally", ["Enemy"]);
    toggleActionButton(actionId, suffix, "Ally");
}

function toggleEnemy(actionId, suffix = "Enemy") {
    handleMutualExclusivity(actionId, "Enemy", ["Ally"]);
    toggleActionButton(actionId, suffix, "Enemy");
}

function toggleVilify(actionId, suffix = "Vilify") {
    toggleActionButton(actionId, suffix, "Vilify");
}

function toggleAmplify(actionId, suffix = "Amplify") {
    handleMutualExclusivity(actionId, "Amplify", ["Radial"]);
    toggleActionButton(actionId, suffix, "Amplify");
}

function toggleRadialVitiate(actionId, suffix = "Radial") {
    handleMutualExclusivity(actionId, "Radial", ["Amplify"]);
    toggleActionButton(actionId, suffix, "Radial");
}
