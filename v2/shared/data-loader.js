...
const DataLoader = {
  ...
  buildLookupMaps() {
    ...
    console.log('🔧 Actions available:', this.cache.actions ? this.cache.actions.length : 'none');
    console.log('🔧 Masteries available:', this.cache.masteries ? this.cache.masteries.length : 'none');
    ...
    if (this.cache.actions) {
      ...
      console.log('🔧 Unique masteries in actions:', this.lookupMaps.actionsByMastery.size);
    }
  ...
  loadActions() {
    if (this.cache.actions) {
      console.log('🔄 DataLoader: Actions already cached, returning', this.cache.actions.length, 'actions');
    ...
    if (window.actionlistV2) {
      console.log('🔄 Loading actionlistV2 with', window.actionlistV2.length, 'actions');
    ...
    if (window.actionlist) {
      console.log('🔄 Loading actionlist with', window.actionlist.length, 'actions');
  ...
  getActionsByMastery(masteryId) {
    ...
    const allActions = this.cache.actions.filter(action => 
    ...
    if (actions.length === 0 && this.lookupMaps.actionsByMastery.size === 0) {
      ...
      const fallbackActions = this.cache.actions.filter(action => 
  ...
  getLookupMapStats() {
    ...
    let count = 0;
    for (const [masteryId, actions] of this.lookupMaps.actionsByMastery.entries()) {
      if (count < 5) {
  ...
  debugLookupMaps() {
    ...
    console.log(`🔍 Test: ${testMastery} has ${testActions ? testActions.length : 0} actions`);
