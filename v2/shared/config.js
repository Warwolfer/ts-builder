...
const Config = {
  ...
  preferences: {
    ...
    get(key) {
      try {
        ...
        return preferences[key] !== undefined ? preferences[key] : this.defaults[key];
      } catch (error) {
  ...
  init() {
    ...
    if (this.features.isEnabled('verboseLogging')) {
      ...
      console.log('Features enabled:', Object.keys(this.features).filter(f => 
        typeof this.features[f] !== 'function' && this.features.isEnabled(f)
      ));
      console.groupEnd();
    }
    ...
    document.dispatchEvent(new CustomEvent('ts:config:ready', {
      detail: { config: this }
    }));
  }
};
