  ...
  generateToggleButtons(action) {
    ...
    if (configs) {
      configs.forEach(config => {
        ...
        if (config.hasInput) {
          buttonHtml += `<input type="text" class="risky-input" placeholder="Modifiers" style="display: none;" oninput="updateRiskySuffix('${action.lookup}')" onkeyup="updateRiskySuffix('${action.lookup}')">`;
        }
    ...
    if (inputs) {
      ...
      if (inputs.maxhp) {
        buttons.push(`<input type="number" class="maxhp-input" placeholder="MaxHP" min="1" onkeydown="validateNumericInput(event)" oninput="updateMaxHPSuffix('${action.lookup}')" onkeyup="updateMaxHPSuffix('${action.lookup}')">`);
      }
    }
...
function toggleActionButton(actionId, suffix, baseText) {
  ...
  const rollCodeElement = cardElement.querySelector('.rollcode');
  const inputElement = cardElement.querySelector('.risky-input');
...
function updateMaxHPSuffix(actionId) {
  ...
  if (maxhpSpan) {
    maxhpSpan.innerHTML = maxhpText || 'MaxHp';
  }
}
    ...
    if (!isCurrentlyActive) {
        mutuallyExclusiveList.forEach(excludedSuffix => {
            ...
            if (excludedButton) {
                ...
                // Remove the excluded suffix from rollcode
                const suffixPattern = new RegExp(` · ${excludedSuffix}`, 'g');
                rollCodeElement.innerHTML = rollCodeElement.innerHTML.replace(suffixPattern, '');
            }
        });
    }
