...
class BuildSheetV2 {
    ...
    calculateSaveBreakdowns() {
        const currentState = this.state.getState();
        const masteries = this.dataLoader.cache.masteries;
        const { chosenMasteries, chosenMasteriesRanks, accessoryType, accessoryRank } = currentState;
        ...
        if (accessoryType && accessoryRank !== undefined) {
            ...
            const accessoryName = accessoryType.charAt(0).toUpperCase() + accessoryType.slice(1) + ' Accessory';
    ...
    calculateStatBreakdowns(stats) {
        const currentState = this.state.getState();
        const actions = this.dataLoader.cache.actions;
        const { armorType, armorRank, chosenActions, chosenMasteriesRanks } = currentState;
        ...
        if (armorType && this.calculations.ARMOR_MULTIPLIERS[armorType]) {
            ...
            if (armorBonus > 0) {
                ...
                hpBreakdown.push(`${armorTypeName} Armor (${rankLabel}): ${armorBonus}`);
            }
        }
        ...
        for (const actionName of chosenActions) {
            ...
            if (action && action.bonuses && action.bonuses.hp) {
                if (action.bonuses.hp === "rank-based" && actionName === "defense") {
                    ...
                    const sturdyMasteryRank = this.calculations.getHighestDefenseMasteryRank(currentState, actions);
                    const baseSturdy = 25;
        ...
        for (const actionName of chosenActions) {
            ...
            if (action && action.bonuses && action.bonuses.movement) {
                if (action.bonuses.movement === "rank-based" && actionName === "speed") {
                    ...
                    const swiftMasteryRank = this.calculations.getHighestOffenseMasteryRank(currentState, actions);
                    let swiftBonus = 0;
    ...
    generateActionCardContent(action, state, masteries) {
        ...
        const swapButton = action.lookup === 'revive' ?
    ...
    generateToggleButtons(action) {
        ...
        if (inputs) {
            if (inputs.target) {
                buttons.push(`<input type="text" class="target-input" placeholder="Target" oninput="updateTargetSuffix('${action.lookup}')" onkeyup="updateTargetSuffix('${action.lookup}')">`);
            }
            if (inputs.maxhp) {
                buttons.push(`<input type="number" class="maxhp-input" placeholder="Max HP" min="1" onkeydown="validateNumericInput(event)" oninput="updateMaxHPSuffix('${action.lookup}')" onkeyup="updateMaxHPSuffix('${action.lookup}')">`);
            }
            if (inputs.bank) {
                buttons.push(`<input type="text" class="target-input" placeholder="Banked Action" min="1" onkeydown="updateTargetSuffix(event)" oninput="updateTargetSuffix('${action.lookup}')" onkeyup="updateTargetSuffix('${action.lookup}')">`);
            }
            if (inputs.damageTaken) {
                buttons.push(`<input type="number" class="damage-taken-input" placeholder="Damage Taken" min="0" onkeydown="validateNumericInput(event)" oninput="updateDamageTakenSuffix('${action.lookup}')" onkeyup="updateDamageTakenSuffix('${action.lookup}')">`);
            }
            if (inputs.overdriveDamage) {
                buttons.push(`<input type="number" class="overdrive-damage-input" placeholder="Overdrive Damage" min="0" onkeydown="validateNumericInput(event)" oninput="updateOverdriveDamageSuffix('${action.lookup}')" onkeyup="updateOverdriveDamageSuffix('${action.lookup}')">`);
            }
        }
    ...
    generateMasteryIcons(action, state, masteries) {
        ...
        if (action.lookup !== 'evolve' && (!action.dice || !action.roll.includes('Mastery'))) {
        ...
        applicableMasteries.forEach(masteryId => {
            ...
            if (mastery) {
                ...
                const isDowncast = action.lookup !== 'attack' && action.lookup !== 'evolve' && this.isActionDowncast(action, mastery);
    ...
    replaceBreakTypes(state) {
        ...
        if (state.chosenMasteries && state.chosenMasteries.length > 0) {
            ...
            const primaryMastery = masteries.find(m => m.lookup === primaryMasteryId);

            if (primaryMastery && primaryMastery.breakType) {
                const breakType = primaryMastery.breakType.charAt(0).toUpperCase() + primaryMastery.breakType.slice(1);
        ...
        if (cardElement) {
            ...
            if (actionElement) {
                ...
                if (rollCodeElement) {
                    ...
                    rollCodeElement.innerHTML = rollCodeElement.innerHTML.replace(existingSuffixPattern, '');
            ...
            if (breakTypeReplace && mastery && mastery.breakType) {
                const breakType = mastery.breakType.charAt(0).toUpperCase() + mastery.breakType.slice(1);
...
function addGlowEffect(element, containerClass) {
    ...
    if (containerClass === 'masterycircle') {
        container = element.closest('.masteryicon') || element.closest('#savesicons');
...
function updateRiskySuffix(actionId) {
    ...
    if (actionId === 'reckless-attack') {
        ...
        if (customText) {
            ...
            const updatedHtml = currentHtml.replace(/(<span class="damagepassivemod">)[^<]*(<\/span>)/, `$1${customText}$2 `);
            console.log('Updated HTML:', updatedHtml);
            rollCodeElement.innerHTML = updatedHtml;
        } else {
            // Clear the damagepassivemod span
            const updatedHtml = currentHtml.replace(/(<span class="damagepassivemod">)[^<]*(<\/span>)/, '$1$2');
        ...
        const suffixPattern = / \. [^.]*$/;
        rollCodeElement.innerHTML = rollCodeElement.innerHTML.replace(suffixPattern, ` . ${newSuffix}`);
    }
}
...
function handleMutualExclusivity(actionId, currentSuffix, mutuallyExclusiveList) {
    ...
    if (!isCurrentlyActive) {
        mutuallyExclusiveList.forEach(excludedSuffix => {
            ...
            const excludedButton = Array.from(cardElement.querySelectorAll('.risky-toggle')).find(btn =>
                btn.textContent === excludedSuffix && btn.classList.contains('active')
            );
            ...
            if (excludedButton) {
                ...
                if (excludedInput) {
                    ...
                    if (excludedSuffix === 'Charging') {
                        ...
                        rollCodeElement.innerHTML = rollCodeElement.innerHTML.replace(chargingSuffixPattern, '');
                    } else if (excludedSuffix === 'Release') {
                        const releaseSuffixPattern = / · Release \([^)]*\)/g;
                        rollCodeElement.innerHTML = rollCodeElement.innerHTML.replace(releaseSuffixPattern, '');
                    }
                }
                ...
                const inputSuffixPattern = new RegExp(` · ${excludedSuffix} \\([^)]*\\)`, 'g');
                rollCodeElement.innerHTML = rollCodeElement.innerHTML.replace(baseSuffixPattern, '');
                rollCodeElement.innerHTML = rollCodeElement.innerHTML.replace(inputSuffixPattern, '');
            }
        });
    }
}
...
function toggleReviveStabilize(buttonElement) {
    ...
    setTimeout(() => {
        ...
        if (isCurrentlyRevive) {
            ...
            if (cardInfoElement) {
                cardInfoElement.innerHTML = "<p>This action may be selected for free when you select 'Heal'.</p><p>Bonus Action: Revive an ally within range. They regain 50% of their max HP.</p>";
            }
            ...
            if (cardInfoElement) {
                cardInfoElement.innerHTML = "<p>This action may be selected for free when you select 'Heal'.</p><p>Bonus Action: Revive an ally within range. They regain 50% of their max HP.</p><p>Limitations: Lander characters cannot use this action card.</p>";
            }

            // Remove "· Stabilize" from the rollcode
            rollCodeElement.innerHTML = rollCodeElement.innerHTML.replace(' · Stabilize', '');
        }
...
function refreshAllActionFilters() {
    ...
    const roleFilterButtons = document.querySelectorAll('.filter-pill[data-filter]');

    roleFilterButtons.forEach(button => {
        const filterType = button.getAttribute('data-filter');
        if (['offense', 'defense', 'support', 'alter'].includes(filterType) && button.classList.contains('active')) {
    ...
    roleFilterButtons.forEach(button => {
        const filterType = button.getAttribute('data-filter');
        if (['main', 'bonus', 'free', 'passive', 'special'].includes(filterType) && button.classList.contains('active')) {
    ...
    actions.forEach(action => {
        ...
        if (actionCard) {
            ...
            if (activeRoleFilters.length > 0) {
                shouldShow = shouldShow && (action.category && activeRoleFilters.includes(action.category));
            }

            // Use filter check (OR within uses)
            if (activeUseFilters.length > 0) {
                shouldShow = shouldShow && (action.use && action.use.some(useType => activeUseFilters.includes(useType)));
            }
